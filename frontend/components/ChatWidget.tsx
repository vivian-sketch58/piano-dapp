        ...
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="bg-orange-500 px-4 py-3 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-orange-100 text-xs">Policy · Product · Tech · General</p>
            </div>
          </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] rounded-xl px-3 py-2 text-sm ${
                  ...
                  {msg.agent && (
                    <p className="text-xs text-orange-400 mb-1">{AGENT_LABELS[msg.agent] ?? msg.agent}</p>
                  )}
          ...
          <div className="p-3 border-t border-gray-700 flex gap-2">
            <input
              ...
              className="flex-1 bg-gray-800 text-white text-sm rounded-lg px-3 py-2 outline-none focus:ring-1 focus:ring-orange-500"
            />
            <button
              onClick={sendMessage}
              disabled={loading || !input.trim()}
              className="bg-orange-500 hover:bg-orange-400 disabled:opacity-50 text-white rounded-lg px-3 py-2 text-sm font-semibold transition"
            >
              Send
            </button>
          </div>
